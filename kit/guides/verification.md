...
The verifier is itself tested: `node --test tests/verify-plugin.test.mjs` runs each of the sixteen
rejection branches `verifyPlugin` can take against a single-field mutant that must fail, asserting the
message that mutant is supposed to produce. **A check that has never been observed rejecting anything is
not evidence.** The `--kit` rung is not covered by those mutants; it is exercised by running it here.
