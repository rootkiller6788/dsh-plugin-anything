 ...
 */
export function applyGitLogTool(ctx: Context, options: GitLogOptions): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const { stdout } = await invoke(
        options.backend,
        ['log', args.target, ...(args.limit === undefined ? [] : ['-n', String(args.limit)])],
...
/**
 * Narrow an opaque `ToolResult['meta']` to the shape {@link applyGitLogTool} projected.
