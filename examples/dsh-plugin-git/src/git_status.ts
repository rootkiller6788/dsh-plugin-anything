 ...
 *
 * @module dsh-plugin-git/git_status
 */
 ...
 */
export function applyGitStatusTool(ctx: Context, options: GitStatusOptions): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const { stdout } = await invoke(
        options.backend,
        ['status', args.target, ...(args.limit === undefined ? [] : ['-n', String(args.limit)])],
...
/**
 * Narrow an opaque `ToolResult['meta']` to the shape {@link applyGitStatusTool} projected.
